...
#include "hidapi.h"
typedef struct hid_device_info* hid_device_info_p;
typedef hid_device* hid_device_p;
typedef wchar_t* wchar_t_p;

typedef hid_device_info_p (__cdecl *hid_proxy_enumerate)(unsigned short, unsigned short);
typedef wchar_t_p (__cdecl *hid_proxy_error)(hid_device*);
typedef int (__cdecl *hid_proxy_exit)();
typedef int (__cdecl *hid_proxy_init)();
typedef int (__cdecl *hid_proxy_free_enumeration)(hid_device_info_p);
typedef hid_device_p (__cdecl *hid_proxy_open)(unsigned short, unsigned short, wchar_t_p);
typedef hid_device_p (__cdecl *hid_proxy_open_path)(const char*);
typedef int (__cdecl *hid_proxy_close)(hid_device_p);
typedef int (__cdecl *hid_proxy_send_feature_report)(hid_device_p, const unsigned char*, size_t);
typedef int (__cdecl *hid_proxy_get_feature_report)(hid_device_p, const unsigned char*, size_t);

hid_proxy_enumerate f1;
hid_proxy_error f2;
hid_proxy_exit f3;
hid_proxy_init f4;
hid_proxy_free_enumeration f5;
hid_proxy_open f6;
hid_proxy_open_path f7;
hid_proxy_close f8;
hid_proxy_send_feature_report f9;
hid_proxy_get_feature_report f10;


HINSTANCE hidApiHandle;
...
int gcn64_init(int verbose)
{
    ...
    hidApiHandle = LoadLibrary(TEXT("hidapi_wineproxy.dll")); 

    if (hidApiHandle != NULL) {
        f1 = (hid_proxy_enumerate) GetProcAddress(hidApiHandle, "hid_proxy_enumerate");
        f2 = (hid_proxy_error) GetProcAddress(hidApiHandle, "hid_proxy_error");
        f3 = (hid_proxy_exit) GetProcAddress(hidApiHandle, "hid_proxy_exit");
        f4 = (hid_proxy_init) GetProcAddress(hidApiHandle, "hid_proxy_init");
        f5 = (hid_proxy_free_enumeration) GetProcAddress(hidApiHandle, "hid_proxy_free_enumeration");
        f6 = (hid_proxy_open) GetProcAddress(hidApiHandle, "hid_proxy_open");
        f7 = (hid_proxy_open_path) GetProcAddress(hidApiHandle, "hid_proxy_open_path");
        f8 = (hid_proxy_close) GetProcAddress(hidApiHandle, "hid_proxy_close");
        f9 = (hid_proxy_send_feature_report) GetProcAddress(hidApiHandle, "hid_proxy_send_feature_report");
        f10 = (hid_proxy_get_feature_report) GetProcAddress(hidApiHandle, "hid_proxy_get_feature_report");
        f4();
        
    } else {
        printf("THERE ARE ERRORS!!!! Status: %ld\n", GetLastError());
    }
...
struct gcn64_info *gcn64_listDevices(struct gcn64_info *info, struct gcn64_list_ctx *ctx)
{
	...
	ctx->devs = f1(OUR_VENDOR_ID, 0x0000);
	if (!ctx->devs) {
...
int gcn64_send_cmd(gcn64_hdl_t hdl, const unsigned char *cmd, int cmdlen)
{
	...
	n = f9(hdev, buffer, sizeof(buffer));
	if (n < 0) {
...
int gcn64_poll_result(gcn64_hdl_t hdl, unsigned char *cmd, int cmd_maxlen)
{
	...
	n = f10(hdev, buffer, sizeof(buffer));
	if (n < 0) {
		fprintf(stderr, "Could not send feature report (%ls)\n", f2(hdev));
		return -1;
	}
